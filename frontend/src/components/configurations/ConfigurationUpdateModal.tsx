...
export default function ConfigurationUpdateModal({config, onClose, onSuccess, onError}: UpdateModalProps) {
    const queryClient = useQueryClient();
    const [name, setName] = useState(config?.name || '');

    const mutation = useMutation({
        mutationFn: () => updateConfiguration(config!.id, {name}),
        onSuccess: async () => {
            await Promise.all([
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/10 backdrop-blur-sm">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
                <h2 className="text-xl font-bold mb-4">Update Configuration</h2>
