...
import {getOrderList, getOrderLists} from '../../api/order_lists';
import {formatDate, formatName} from '../../utils/formatters';
import type {SortDirection} from '../../types/common';
import type {OrderListSingleGetResponseDTO, OrderListSortField, OrderListType} from '../../types/order_lists';
...
import OrderListInfoModal from './OrderListInfoModal';

interface Props {
    isOpen: boolean;
    onClose: () => void;
    onSelect: (id: number, data: OrderListSingleGetResponseDTO) => void;
}
...
export default function OrderListSelectModal({isOpen, onClose, onSelect}: Props) {
    const tableContainerRef = useRef<HTMLDivElement>(null);
    const queryClient = useQueryClient();

    const [page, setPage] = useState(1);
    const [sortBy, setSortBy] = useState<OrderListSortField>('updated_at');
    const [sortDir, setSortDir] = useState<SortDirection>('desc');
    const [searchInput, setSearchInput] = useState('');
    const [appliedSearch, setAppliedSearch] = useState('');
    const [selectedFilters, setSelectedFilters] = useState<Record<string, string[]>>({});

    const [infoOrderListId, setInfoOrderListId] = useState<number | null>(null);

    const pageSize = 5;

    const {data, isLoading, isPlaceholderData} = useQuery({
        queryKey: ['order_lists_select', {page, pageSize, sortBy, sortDir, appliedSearch, types: selectedFilters.type}],
        queryFn: () => getOrderLists(page, pageSize, sortBy, sortDir, appliedSearch, selectedFilters.type),
    ...
    const handleSort = (field: string) => {
        ...
        if (sortBy === typedField) setSortDir(sortDir === 'asc' ? 'desc' : 'asc');
        else {
    ...
    return (
        <>
            <div className="fixed inset-0 z-70 flex items-center justify-center p-6 bg-black/40 backdrop-blur-[1px]"
                ...
                <div
                    ...
                    <div
                        ...
                        <h2 className="text-xl font-bold text-gray-900">Select Order List</h2>
                    ...
                    <div className="flex-1 min-h-0 p-4 bg-gray-50/50">
                        ...
                        >
                            {isLoading ? (
                                <tr>
                                    <td colSpan={5} className="text-center py-10 text-gray-500">Loading order lists...
                                    </td>
                                </tr>
                            ) : !data?.order_lists?.length ? (
                                <tr>
                                    <td colSpan={5} className="text-center py-10 text-gray-500">No order lists found.
                                    </td>
                                </tr>
                            ) : (
                                data.order_lists.map((orderList: OrderListSummary) => (
                                    <tr key={orderList.id}
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm flex gap-2">
                                            <button
                                                onClick={() => setInfoOrderListId(orderList.id)}
                                            ...
                                            <button
                                                onClick={async () => {
                                                    const fullData = await queryClient.fetchQuery({
                                                        queryKey: ['order_list', orderList.id],
                                                        queryFn: () => getOrderList(orderList.id)
                                                    });
                                                    onSelect(orderList.id, fullData);
                                                }}
                                                ...
                                                title="Select this order list"
                                            >
                                                <CheckIcon className="w-5 h-5"/>
                                            </button>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{formatName(orderList.name)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 capitalize">{orderList.type}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(orderList.created_at)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(orderList.updated_at)}</td>
                                    </tr>
                                ))
                            )}
                        </DataTable>
            ...
            <OrderListInfoModal orderListId={infoOrderListId} onClose={() => setInfoOrderListId(null)}/>
        </>
    );
}